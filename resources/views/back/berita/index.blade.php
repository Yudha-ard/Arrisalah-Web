                  ...
                  <div class="table-responsive">
                    <table class="table table-bordered">
                      ...
                      <tbody>
                        @foreach($data as $row)
                          <tr>
                             <td><img src="{{ URL::to('/') }}/img/berita_img/{{ $row->gambar }}" class="img-thumbnail" width="75" /></td>
                             <td>{{ $row->judul }}</td>
                             <td>{!! html_entity_decode(str_limit($row->konten, 20, ' ...')) !!}</td>
                             <td>{{ $row->kategori->nama }}</td>
                             <td>
                                <a href="{{ route('berita.edit', $row->id) }}" class="btn btn-sm btn-primary">Edit</a></td>
                             <td>   
                                <form class="" action="{{ route('berita.destroy', $row->id) }}" method="post">
                                  {{ csrf_field() }}
                                  {{ method_field('DELETE') }}
