            ...
            <h1 class="h3 mb-2 text-gray-800">Pengajar</h1>
              <div class="card shadow mb-4">
                <div class="col-sm-12 col-md-12 mt-3">
                  <a href="{{  route('pengajar.create') }}" class="btn btn-success btn-icon-split">
                  ...
                  <div class="table-responsive">
                    <table class="table table-bordered">
                      ...
                      <tbody>
                        @foreach($data as $row)
                          <tr>
                            <td><img src="{{ URL::to('/') }}/img/pengajar_img/{{ $row->foto }}" class="img-thumbnail" width="75" /></td>
                            <td>{{ $row->nama }}</td>
                            <td>{{ str_limit($row->keterangan, 10, ' ...') }}</td>
                            <td>{{ $row->kategori->nama }}</td>
                            <td>
                                <a href="{{ route('pengajar.edit', $row->id) }}" class="btn btn-sm btn-primary">Edit</a>
                            </td>
                            <td>    
                                <form class="" action="{{ route('pengajar.destroy', $row->id) }}" method="post">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
